...
- This is a multimodal/visual validation agent and must stay on `sonnet -> mimo-v2.5[1m]`.
- Use this agent only for screenshot/UI visual validation, design consistency checks, accessibility visual checks, or other visual evidence review.
- For OCR or general image description, call `image-reader` first, then use this agent to judge visual quality from the structured visual evidence.
